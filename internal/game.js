...
const optionStack = [];
window.optionStack = optionStack
							...
							optionStack.addOption({
								...
								"3": async () => {
									if (planet.historicalDevelopment) {
										await typeMessage("There have been significant developments since we've last visited. We need to research the changes before proceeding.")
										return
									}
									menuStack.addMenu(
										"Catalyze",
				...
				"3": async () => {
					menuStack.addMenu(
						...
						async () => {
							...
							await typeMessage("TODO: blah blah do increments of 1000")
						}
					)
					optionStack.addOption({
						"default": async (msg) => {
							...
							let developments = 0;
							for (let planet of DATA.planets) {
								if (planet.research.complete === false) continue;
								planet.yearsSinceLastHistoricalDevelopment += num
								planet.year += num
								if (planet.historicalDevelopment === false && Math.random() < .1 * planet.yearsSinceLastHistoricalDevelopment) {
									planet.historicalDevelopment = true
									developments++
								}
								await typeMessage(`(There have been ${developments} historical development${developments>1?"s":""})`)
							}
							await prev()
						},
						"back": async () => { await prev() }
